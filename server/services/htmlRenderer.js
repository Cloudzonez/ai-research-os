...
const HTML_PROMPT_ZH = `你是一位学术出版专家。请根据以下论文信息，生成一个完整的、美观的 HTML 页面，适合研究人员阅读。
...
const HTML_PROMPT_EN = `You are an academic publishing specialist. Generate a complete, beautiful HTML page based on the following paper information, suitable for researchers to read.
...
export async function generatePaperHTML(paper, locale = "zh") {
  const systemPrompt = locale === "zh" ? HTML_PROMPT_ZH : HTML_PROMPT_EN;
  ...
  try {
    const result = await chat(
      [
        ...
        { role: "user", content: `请为以下论文生成一个美观的 HTML 页面：\n\n${paperInfo}` },
      ],
      locale,
...
export async function generateDigestHTML(papers, options = {}) {
  ...
  const systemPrompt = locale === "zh"
    ? `你是一位学术出版专家。请根据提供的论文列表，生成一个完整的每日论文摘要 HTML 页面。设计现代、干净，适合学术阅读。包含目录（按类别分组）、每篇论文的 TL;DR 和链接。返回完整 HTML 文档。`
    : `You are an academic publishing specialist. Generate a complete daily paper digest HTML page from the provided paper list. Modern, clean design for academic reading. Include a table of contents (grouped by category), TL;DR for each paper, and links. Return complete HTML document.`;
